...
- **關鍵判斷問題**：「執行方法（how）是否需要被探索？還是已知？」——如果方法已知，直接執行即可；如果需要先推論出步驟，才需要 Planning
- **計劃的本質**：計劃是**起點，不是不可更動的腳本**（starting point, not a rigid script）；Agent 應能在執行途中根據新資訊調整計劃，這正是 Planning 與硬編碼流程的核心差異——**適應性（adaptability）**
- **彈性 vs 可預測性的取捨**：Planning 賦予 Agent 靈活應對突發情況的能力，但也因此讓行為較難預測與測試；相對地，固定流程可預測但缺乏應對變化的能力
